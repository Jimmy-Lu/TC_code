...
from .operations.edge_softmax import edge_softmax
...
class GATConv(nn.Module):
    ...
    def forward(self, graph, feat):
        ...
        with graph.local_scope():
            ...
            # compute softmax
            graph.edata['a'] = self.attn_drop(self.softmax(graph, e))
            # message passing
...
#     # x = th.nn.Parameter(th.ones_like(g.ndata['feat']), requires_grad=True)
#     x = torch.ones(g.ndata['feat'].shape)
#     print(x)
#     model = GAT_DGL(g)
#     # model.eval()
#     y = model(x)
