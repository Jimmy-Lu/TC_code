...
class Node():
    ...
    def __init__(self, uid, name, op, input, output, output_shape=None, params=None):
        """
        ...
        self.attr = []  # e or v
        self.symbol = []  # src or dst or edge or weight
        self.symbol_order = [None, None, None, None]  # src,dst,edge,weight
        self._caption = ""
...
def symbol_order(hl_graph, id):
    symbols = hl_graph.nodes[id].symbol
    order = hl_graph.nodes[id].symbol_order
    new_symbol = []
    if symbols == ['Src']:
        new_symbol.append('Src' + str(order[0]))
    elif 'Dst' in symbols:
        new_symbol.append('Dst' + str(order[1]))
    elif symbols == ['Edge']:
        new_symbol.append('Edge' + str(order[2]))
    else:
        new_symbol.append('W' + str(order[3]))
    return new_symbol


def shape_output(hl_graph, id):
    shape = hl_graph.nodes[id].output_shape
    symbols = hl_graph.nodes[id].symbol
...
# generate staged code(scatter, gather, apply)
def staged_code(hl_graph, name):
    stage_pre = ''
    current_layer = []
    visited_store = []
    visited_load = []
    id_pre = hl_graph.id_list[0]
    tplt = "{0:^20}\t{1:^60}\t{2:^60}\t\n"
    with open("result/{}.model".format(name), "w") as f:
        f.writelines(tplt.format("opname", "shapes", "symbol"))
        for id in hl_graph.id_list:
            op = str(hl_graph.nodes[id].op)
            # correct gather op output tensor type
            if op[:6] == "Gather":
                hl_graph.nodes[id].symbol.remove('Edge')
                hl_graph.nodes[id].symbol.append('Dst')
            # stage generate
            if id in hl_graph.scatter_stage:
                stage = 'Scatter:'
            elif id in hl_graph.gather_stage:
                stage = 'Gather:'
            elif id in hl_graph.apply_stage:
                stage = 'Apply:'
            else:
                stage = 'Input:'
            # not output input tensors and weight
            if stage != stage_pre and stage != 'Input:' and op != 'input0':
                f.writelines("\n" + stage + "\t\n")
            stage_pre = stage
            if hl_graph.nodes[id_pre].op[:6] == "Gather" and stage == "Gather:":
                f.writelines("\nGather:\t\n")
            id_pre = id
            # symbol and shape generate
            if hl_graph.nodes[id].input is not None:
                symbol = (str(symbol_order(hl_graph, id)) + "," + str(
                    [symbol_order(hl_graph, input) for input in
                     hl_graph.nodes[id].input])).replace("'", "")
                shape = (str(shape_output(hl_graph, id)) + "," + str(
                    [shape_output(hl_graph, input) for input in
                     hl_graph.nodes[id].input])).replace("'", "")
            else:
                symbol = str(symbol_order(hl_graph, id)).replace("'", "")
                shape = str(shape_output(hl_graph, id)).replace("'", "")

            for layer in hl_graph.layers:
                if id in layer:
                    current_layer = layer

            # output op code line and not output input tensors and weight
            if stage != 'Input:' and op != 'input0':
                # add load
                if hl_graph.nodes[id].input is not None:
                    for input in hl_graph.nodes[id].input:
                        if input not in current_layer and input not in visited_load:
                            visited_load.append(input)
                            op_load = ('LD_' + str(symbol_order(hl_graph, input)[0])).replace("'", "")
                            shape_load = ""
                            symbol_load = str(symbol_order(hl_graph, input)).replace("'", "")
                            f.writelines(tplt.format(op_load, shape_load, symbol_load))

                f.writelines(tplt.format(op, shape, symbol))

                # add store
                for id_ in hl_graph.id_list:
                    if hl_graph.nodes[id_].input is not None:
                        if id in hl_graph.nodes[id_].input and id_ not in current_layer and id not in visited_store:
                            visited_store.append(id)
                            op_store = ('ST_' + str(symbol_order(hl_graph, id)[0])).replace("'", "")
                            shape_store = ""
                            symbol_store = str(symbol_order(hl_graph, id)).replace("'", "")
                            f.writelines(tplt.format(op_store, shape_store, symbol_store))
...
class Graph():
    ...
    def add_node(self, node):
        ...
        if node.input is None or node.input == []:
            ...
            for input in node.input:
                node.input_shape.append(self.nodes[input].output_shape)
                if self.nodes[input].attr[0] == 'T':
                    attr = 'V'
                elif self.nodes[input].op[:6] == 'Gather':
                    attr = 'V'
                else:
                    attr = self.nodes[input].attr[0]
            if node.op[:7] == 'Scatter':
                node.attr.append('E')
            else:
    ...
    def add_input_symbol(self, node, count):
        if count != 1:
            node.symbol = ["W"]
            node.symbol_order[3] = count - 2
