...
void Sparse::generate_tiles() {
    for (vid_t pid = thread_id; pid < gl->num_partitions; pid += Config::num_workers) {
        const auto cols_curr = gl->partitions[pid];
        vid_t row_curr = 0;
        
        while (row_curr < num_vertices) {
            select_rows(row_curr, cols_curr);
            if (tiles.size() >= max_tile) save_tiles();
        }
        
        // save tiles to files
        if (!tiles.empty()) {
            tiles.back()->output = true;
            save_all_tiles();
        } else continue;
    }
}

void Sparse::select_rows(vid_t &row_curr, const GraphLoader::partition_t &cols_curr) {
    auto[p_begin, p_end, pid] = cols_curr;
    auto *tile = new Tile_T();
    tile->output = false;
    tile->tile_id = num_tiles;
    
    // destination
    tile->target_vertex_begin = p_begin;
    tile->target_vertex_total = p_end - p_begin;
    tile->target_vertex_partition_id = pid;
    
    // source
    tile->source_vertex_list.resize(Config::tile_max_src_edge);
    while (row_curr < num_vertices
    ...
    if (tile->forward_edge_total == 0) {
        if (row_curr == num_vertices) {
            ...
            // only one row of edges will reach the tile size threshold
            assert(gl->adj_csr_size[row_curr][pid] * Config::num_element_feat_edge
                   + Config::num_element_feat_src > Config::num_total_element_tile);
            
            tile->forward_edge_total = int((Config::num_total_element_tile - Config::num_element_feat_src)
...
void Sparse::tile_saver(Tile_T *tile, EFormat tile_format,
    ...
    if (tile_format == EFormat::ascii) {
        ...
        //scalar
        outfile_a << "tile_id\n";
        outfile_a << tile->tile_id << "\n";
        outfile_a << "output\n";
        outfile_a << tile->output << "\n";
        outfile_a << "target_vertex_partition_id\n";
        outfile_a << tile->target_vertex_partition_id << "\n";
        ...
        // outfile_a << tile->backward_edge_total << "\n";
        ...
        //scalar
        Config::string_2_binary("tile_id", outfile_b);
        Config::scalar_2_binary(tile->tile_id, outfile_b);
        Config::string_2_binary("output", outfile_b);
        Config::scalar_2_binary(tile->output, outfile_b);
        Config::string_2_binary("target_vertex_partition_id", outfile_b);
        Config::scalar_2_binary(tile->target_vertex_partition_id, outfile_b);
        ...
        // Config::scalar_2_binary(tile->backward_edge_total, outfile_b);
        ...
        // Config::vector_2_binary(tile->backward_edge_idx, outfile_b);
        // Config::string_2_binary("backward_edge_type", outfile_b);
        // Config::vector_2_binary(tile->backward_edge_type, outfile_b);
        // Config::string_2_binary("backward_target_idx", outfile_b);
        // Config::vector_2_binary(tile->backward_target_idx, outfile_b);
        // Config::string_2_binary("backward_source_idx", outfile_b);
        // Config::vector_2_binary(tile->backward_source_idx, outfile_b);
