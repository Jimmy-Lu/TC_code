...
namespace ramulator {
    ...
    public:
        ...
        long get_num_cycle() const { return num_dram_cycles.value(); }
        ...
        Memory(
            ...
            // init_mapping_with_file();
            if (type != Type::RoBaRaCoCh) throw runtime_error("");
        ...
        void init_mapping_with_file() {
            ifstream file("../configs/ramulator-mapping/hbm.map");
        ...
        void apply_mapping(long addr, std::vector<int> &addr_vec) {
            ...
            addr_bits[int(T::Level::Row)] = min((int) sizeof(int) * 8,
        ...
        int pending_requests() {
            int reqs = 0;
            for (auto ctrl: ctrls)
                reqs += ctrl->readq.size() + ctrl->writeq.size() + ctrl->otherq.size() + ctrl->actq.size() +
                        ctrl->pending.size();
            return reqs;
        }
