...
EPartitioning Config::name_partitioning = EPartitioning::sparse;
EReorder Config::name_reorder = EReorder::none;
EFormat Config::shard_format = EFormat::binary;
...
int Config::shard_max_element = -1;
int Config::shard_max_src_edge = -1;
int Config::shard_max_dst = -1;
int Config::num_vu = 1;
int Config::num_mu = 1;
...
void Config::update_simulator_config() {
    ...
    shard_max_dst = int(double(SIZE_DST_MEMORY_MB) / SIZE_ELEMENT_BYTE * 1024 * 1024 / num_element_feat_dst);
    shard_max_element = int(double(SIZE_SRC_MEMORY_MB) / Config::SIZE_ELEMENT_BYTE * 1024 * 1024) / num_thread;
    shard_max_src_edge = ceil(double(shard_max_element) / Config::SIZE_FEATURE_ELEMENT);
...
void Config::read_simulator_argument(int argc, char *const *argv) {
    ...
    program.add_argument("-t", "--thread").help("number of shard threads.");
    ...
    program.add_argument("-C", "--coalesce").help("enable row/shard coalesce.");
    program.add_argument("-V", "--vu-balance").help("enable vu intra-shard balance.");
    ...
    program.add_argument("-p", "--partitioning").help("target graph partitioning method.");
    ...
    get(program, "-p", name_partitioning, string_to_partitioning);
    get(program, "-f", shard_format, string_to_format);
...
void Config::print_config() {
    ...
    printf("partitioning=%s, ", partitioning_to_string.at(name_partitioning).c_str());
    printf("format=%s.\n", format_to_string.at(shard_format).c_str());
    printf("s-src-edge=%s, ", to_string(shard_max_src_edge).c_str());
    printf("s-dst=%s.\n", to_string(shard_max_dst).c_str());
    printf("------------------------\n");
