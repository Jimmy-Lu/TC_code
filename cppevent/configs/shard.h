...
#ifndef CPPMAIN_SHARD_H
#define CPPMAIN_SHARD_H

#include <cmath>
#include <vector>
#include "config.h"

#define SHARD_READABLE

using namespace std;

class Shard_T { // column-major
public:
    id_shard_t shard_id = -1;  // 16 PreEdge
    bool output = false;  // 1 PreEdge
    
    id_interval_t target_interval_id = -1;  // 16 to PreEdge
