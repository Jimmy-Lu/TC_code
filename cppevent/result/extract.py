...
partitionings = ['sparse', 'hygcn', 'baseline']
...
class ResultStructure(object):
    def __init__(self, filename: str):
        ...
        print('partitioning:', self.partitioning, end=', ')
...
def extract_file_content(filename: str, worksheet: xlwt.Worksheet):
    ...
    for line in lines:
        ...
        ####################################################
        if 'dataset=' in line and 'partitioning=' in line:
            assert line == 'dataset={}, partitioning={}, format=binary.'.format(rs.dataset, rs.partitioning)
        ####################################################
        if 'model=' in line:
    ...
    worksheet.write(CNT, cid_partitioning, rs.partitioning, cell_style_content)
...
def header(worksheet):
    ...
    worksheet.write(CNT, cid_partitioning, 'Tiling', cell_style_head)
