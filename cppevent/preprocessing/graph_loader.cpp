...
void GraphLoader::create_folder() {
    ...
    if (!fs::exists(path_partition)) fs::create_directory(path_partition);
    path_partition /= string("partitioning.") + partitioning_to_string.at(Config::name_partitioning);
    ...
    // remove old partitioning results in the folder.
...
void GraphLoader::generate_intervals() {
    int size_partition_row = Config::shard_max_dst;
    num_intervals = ceil(double(num_vertices) / size_partition_row);
    intervals = new interval_t[num_intervals];
    for (int i = 0; i < num_intervals; i++) {
        intervals[i] = {i * size_partition_row, (i + 1) * size_partition_row, i};
        if ((i + 1) * size_partition_row >= num_vertices) break;
    }
    get<1>(intervals[num_intervals - 1]) = num_vertices;
    assert(num_vertices >= get<0>(intervals[num_intervals - 1]) && "last partition\n");
}
...
void GraphLoader::load_adj() {
    ...
    fs::path target_path = path_partition / string("meta.shard");
...
void GraphLoader::start_partitioning(int thread_id) {
    ShardGenerator *tg;
...
void GraphLoader::save_coo(GraphLoader::adj_csc_v_t *adj, vid_t num_v) {
    ...
    target_path /= string("partitioning.") + partitioning_to_string.at(Config::name_partitioning);
...
void GraphLoader::generate_adj(const COO *coo, const vector<vid_t> *new_ids) {
    ...
    for (vid_t v_id = 0; v_id < num_vertices; v_id++) {
        adj_csr_size[v_id].resize(num_intervals);
        for (int p_id = 0; p_id < num_intervals; p_id++)
    ...
    for (int e = 0; e < num_edges; e++) {
        ...
        adj_csr_size[src][dst / Config::shard_max_dst]++;
