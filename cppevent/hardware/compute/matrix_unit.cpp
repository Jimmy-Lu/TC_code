...
Op *MatrixUnit::fetch_op_from_ctrl() {
    if (ctrl->queue_issue_matrix.empty()) {
        return nullptr;
    } else {
        auto ret = ctrl->queue_issue_matrix.front();
        ctrl->queue_issue_matrix.pop_front();
        return ret;
    }
}
...
void MatrixUnit::_issue(num_op_t &ops, num_access_t &accesses) {
    if (op_target->name == ISA::GEMM) {
        ...
        shard_coalesced tiles = sh->get_shards_by_ids(op_target->shard_ids);
...
void MatrixUnit::latency_SA_BMM_output(
    ...
    for (auto shard: shards) {
        for (int etype_curr = 0; etype_curr < num_etype; etype_curr++) {
            M_curr = 0;
            for (int i = 0; i < shard->forward_edge_total; i++)
                if (shard->forward_edge_type[i] == etype_curr) M_curr++;
