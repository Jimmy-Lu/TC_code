...
void helps::print_statistics(ArchTop *arch) {
    ...
    long num_shards = arch->ctrl->cnt_shard_processed;
    long coalesced = arch->ctrl->cnt_threads_launched;
    printf("  Cycle#: %.3lf ms, Shard#: %ld, Coalesced: %ld\n", num_cycles_ms, num_shards, coalesced);
    if (num_shards != arch->mah->get_num_shard()) throw runtime_error("shard number.");
