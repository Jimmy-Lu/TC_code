...
namespace ramulator {
    ...
    public:
        ...
        // Constructor
        DRAM(T *spec, bool print, typename T::Level level);
        ~DRAM();
    ...
    void DRAM<T>::regStats(const std::string &identifier) {
        ...
        refresh_cycles
            ...
            .desc(
                "(All-bank refresh only, only valid for rank level) The sum of cycles that is under refresh per memory cycle for level " +
        ...
        busy_cycles
            ...
            .desc(
                "(All-bank refresh only. busy cycles only include refresh time in rank level) The sum of cycles that the DRAM part is active or under refresh for level " +
                identifier + "_" + to_string(id))
            .precision(0);
        active_refresh_overlap_cycles
            .name("active_refresh_overlap_cycles" + identifier + "_" + to_string(id))
            .desc(
                "(All-bank refresh only, only valid for rank level) The sum of cycles that are both active and under refresh per memory cycle for level " +
                identifier + "_" + to_string(id))
            .precision(0)
            .flags(Stats::nozero);
        serving_requests
            ...
            .desc(
                "The sum of read and write requests that are served in this DRAM element per memory cycle for level " +
                identifier + "_" + to_string(id))
            .precision(0);
        average_serving_requests
            .name("average_serving_requests" + identifier + "_" + to_string(id))
            .desc(
                "The average of read and write requests that are served in this DRAM element per memory cycle for level " +
                identifier + "_" + to_string(id))
            .precision(6);
    ...
    // Constructor
    template<typename T>
    DRAM<T>::DRAM(T *spec, bool print, typename T::Level level) :
        ...
        for (int i = 0; i < child_max; i++) {
            DRAM<T> *child = new DRAM<T>(spec, print, typename T::Level(child_level));
            child->parent = this;
            child->id = i;
    ...
    void DRAM<T>::update_timing(typename T::Command cmd, const int *addr, long clk) {
        ...
        for (auto &t : timing[int(cmd)]) {
            ...
            if (print) printf(" %d-%d-%d-%d-%d-k\n", int(level), int(cmd), past, t.val, next[int(t.cmd)]);
            next[int(t.cmd)] = max(next[int(t.cmd)], future); // update future
